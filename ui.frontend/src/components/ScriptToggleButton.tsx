...
const ScriptToggleButton: React.FC<ScriptToggleButtonProps> = ({ type, selectedKeys, onToggle }) => {
  ...
  const handleConfirm = async () => {
    setIsLoading(true);
    const action = type === ScriptType.SCHEDULE ? 'disable' : 'enable';
  ...
  return (
    <DialogTrigger isOpen={toggleDialogOpen} onOpenChange={setToggleDialogOpen}>
      <Button variant={type === ScriptType.SCHEDULE ? 'negative' : 'accent'} style="fill" isDisabled={selectedKeys.length === 0} onPress={() => setToggleDialogOpen(true)}>
        {type === ScriptType.SCHEDULE ? <Cancel /> : <Checkmark />}
        <Text>{type === ScriptType.SCHEDULE ? 'Disable' : 'Enable'}</Text>
      </Button>
