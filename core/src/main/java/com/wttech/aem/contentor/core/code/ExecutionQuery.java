...
public class ExecutionQuery {
  ...
  protected String toSql() {
    ...
    filters.add(
        String.format(
            "s.[%s] = '%s'",
            JcrResourceConstants.SLING_RESOURCE_TYPE_PROPERTY, HistoricalExecution.RESOURCE_TYPE));
    if (path != null) {
    ...
    if (startDate != null) {
      filters.add(
          String.format("s.[startDate] >= CAST('%s' AS DATE)", DateUtils.toString(startDate)));
    }
    if (endDate != null) {
      filters.add(String.format("s.[endDate] <= CAST('%s' AS DATE)", DateUtils.toString(endDate)));
    }
    String where =
        filters.stream().map(f -> "(" + f + ")").reduce((f1, f2) -> f1 + " AND " + f2).orElse("");
