...
@Component(service = TeamsFactory.class)
@Designate(ocd = TeamsFactory.class, factory = true)
public class TeamsFactory {
    ...
    private final Map<String, Teams> factored = new ConcurrentHashMap<>();

    @Activate
    @Modified
    public void activate(Map<String, Object> props, Config config) {
        factored.put(getConfigPid(props), new Teams(config.id(), config.webhookUrl(), config.enabled()));
    }
    ...
    public Collection<Teams> getFactored() {
    ...
    public Teams getById(String id) {
        return factored.values().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new TeamsException(
                        String.format("Cannot find Teams with id '%s'! Ensure that it is configured properly.", id)));
    }
