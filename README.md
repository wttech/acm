...
The following example of the automatic script demonstrates how to generate a CSV report as an output file using the `outputs` [service](https://github.com/wttech/acm/blob/main/core/src/main/java/dev/vml/es/acm/core/code/Outputs.java).

<img src="docs/screenshot-content-script-outputs.png" width="720" alt="ACM Content Script Outputs">

There is no limitation on the number of output files that can be generated by a script. Each output file can have its own label, description, and download name. All outputs are persisted in the history of script executions, allowing you to review and download them later.
