...
public class HistoricalExecution implements Execution, Comparable<HistoricalExecution> {
  ...
  public HistoricalExecution(Resource resource) {
    ...
    this.executable =
        new Code(
            props.get("executableId", String.class), props.get("executableContent", String.class));
  ...
  public static boolean check(Resource resource) {
    return Optional.ofNullable(resource).filter(r -> r.isResourceType(RESOURCE_TYPE)).isPresent();
  }
