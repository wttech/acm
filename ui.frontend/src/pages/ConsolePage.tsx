...
const ConsolePage = () => {
  ...
  const [compiling, syntaxError, compileError] = useCompilation(code);
  ...
  const onExecute = async (args: ArgumentValues) => {
    ...
    try {
      ...
      localStorage.setItem(StorageKeys.EXECUTION_ID, queuedExecution.id);
  ...
  const pollExecutionState = async (jobId: string) => {
    try {
      ...
      if (!isExecutionPending(queuedExecution.status)) {
        localStorage.removeItem(StorageKeys.EXECUTION_ID);
  ...
  useInterval(
    () => {
      const storedJobId = localStorage.getItem(StorageKeys.EXECUTION_ID);

      if (storedJobId) {
        setExecuting(true);
        pollExecutionState(storedJobId);
      } else if (execution && isExecutionPending(execution.status)) {
        pollExecutionState(execution?.id);
      }
    },
    localStorage.getItem(StorageKeys.EXECUTION_ID) || (execution && isExecutionPending(execution.status)) ? executionPollInterval : null,
  );
