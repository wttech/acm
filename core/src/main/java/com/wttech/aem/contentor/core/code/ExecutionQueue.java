...
public class ExecutionQueue implements JobExecutor {
  ...
  public Optional<Execution> read(String jobId, ResourceResolver resourceResolver)
      throws ContentorException {
    Execution result = new ExecutionHistory(resourceResolver).read(jobId).orElse(null);
    if (result == null) {
      result =
          Optional.ofNullable(jobManager.getJobById(jobId)).map(QueuedExecution::new).orElse(null);
    }
