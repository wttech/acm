...
public class ExecutionQueue implements JobExecutor {
  ...
  public JobExecutionResult process(Job job, JobExecutionContext context) {
            ...
            () -> {
              try {
                ...
                throw new ContentorException(
                    String.format("Execution failed asynchronously '%s'", queuedExecution), e);
              }
            });
    ...
    try {
      ...
      return context
          .result()
          .message(QueuedExecution.jobResultMessage(ExecutionStatus.ABORTED))
          .cancelled();
    } catch (Exception e) {
    ...
    try (ResourceResolver resolver = ResourceUtils.serviceResolver(resourceResolverFactory);
        OutputStream outputStream =
            Files.newOutputStream(
                ExecutionFile.path(execution.getJob().getId(), ExecutionFile.OUTPUT))) {
      ...
      throw new ContentorException(
          String.format("Cannot write to files for execution '%s'", execution.getId()), e);
    }
  }
