...
public class QueuedExecution implements Execution {
  ...
  static String composeJobResultMessage(Execution execution) throws ContentorException {
    try {
      ...
      props.put(
          "duration",
          String.valueOf(execution.getDuration())); // JSON uses integers (avoid long -> int)
  ...
  public String getError() {
    return ExecutionQueue.readFile(job.getId(), ExecutionQueue.FileType.ERROR).orElse(null);
  }

  @Override
  public String getOutput() {
    return ExecutionQueue.readFile(job.getId(), ExecutionQueue.FileType.OUTPUT).orElse(null);
  }
