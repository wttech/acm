...
public class ExecutionHistory {
  ...
  public void save(ImmediateExecution execution) throws ContentorException {
    ...
    try {
      String dirPath =
          root.getPath() + "/" + StringUtils.substringBeforeLast(execution.getId(), "/");
      Resource dir =
      ...
      throw new ContentorException(
          String.format("Failed to save execution '%s'", execution.getId()), e);
    }
  }
  ...
  private Resource getOrCreateRoot() throws ContentorException {
    try {
      ...
      throw new ContentorException(
          String.format("Failed to get or create execution history root '%s'", ROOT), e);
    }
  }
  ...
  public Stream<Execution> findAll(ExecutionQuery query) {
    ...
    Stream<Resource> entries =
        StreamUtils.asStream(resourceResolver.findResources(sql, Query.JCR_SQL2));
    return entries
