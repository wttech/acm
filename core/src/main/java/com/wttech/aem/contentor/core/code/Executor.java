...
public class Executor {
  ...
  @Reference private History history;
  ...
  public Execution execute(Executable executable, ExecutionContext context)
    ...
    if (simpleOutputActive) {
      context.setOutputStream(
          new WriterOutputStream(new StringBuilderWriter(simpleOutput), StandardCharsets.UTF_8));
    }
    ...
    try {
      ...
      if (!runnable) {
        return new ImmediateExecution(
            ...
            simpleOutputActive ? simpleOutput.toString() : null,
            null);
      }
